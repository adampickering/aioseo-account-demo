	...
	<div class="space-y-10">
		...
		<div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
			...
			<div class="lg:col-span-2 space-y-8">
				...
				<div class="aio-card">
					<div class="flex items-center gap-4">
						...
						/>
						<div class="min-w-0">
							<h2 class="text-2xl font-heading font-semibold text-text-primary truncate">
								{{ fullName }}
							</h2>
							<div class="flex items-center gap-1.5 mt-1">
								...
								<router-link
									to="/profile"
									class="shrink-0 text-text-light hover:text-brand-blue transition-colors duration-200"
									title="Edit profile"
								>
				...
				<!-- Subscriptions -->
				<div>
					<h3 class="text-lg font-heading font-semibold text-text-primary mb-4">Subscriptions</h3>
					<div class="space-y-3">
						<div
						...
						>
							<div class="min-w-0">
								<div class="flex items-center gap-2">
									<span class="text-sm font-medium text-text-primary truncate">{{ sub.product }}</span>
									<StatusBadge :status="sub.status" />
								</div>
								<p class="text-xs text-text-muted mt-0.5">
									{{ sub.status === 'active' ? 'Renews' : 'Expires' }} {{ formatDate(sub.renewsAt) }}
								</p>
							</div>
						</div>
					</div>
					<router-link
						to="/billing"
						class="inline-flex items-center gap-1 text-sm font-medium text-brand-blue hover:underline transition-all duration-200 mt-3"
					>
				...
				<div v-if="hasCredits">
					...
					<div class="bg-white border border-border rounded-card shadow-card p-5">
						<div class="flex items-center gap-3 mb-4">
							...
							<div>
								<span class="text-2xl font-semibold text-text-primary">{{ formatNumber(remaining) }}</span>
								<span class="text-sm text-text-secondary ml-1.5">Total Available</span>
							</div>
						</div>
						...
						<div class="space-y-1.5 text-sm text-text-secondary">
							...
							<p v-if="credits.paidCredits > 0">{{ formatNumber(credits.paidCredits) }} PAID credits</p>
						</div>

						<router-link
							to="/downloads/ai-credits"
							class="inline-flex items-center gap-1 text-sm font-medium text-brand-blue hover:underline transition-all duration-200 mt-4"
						>
							Add More Credits
							<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								<path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
			...
			<div class="lg:col-span-3">
				<!-- Promo offers (elite users) -->
				<div v-if="showPromos" class="space-y-5">
					<h3 class="text-xl font-heading font-semibold text-text-primary">Special Offers for You!</h3>
					<div class="space-y-3">
						<PromoOfferCard
		...
		<div>
			<h3 class="text-xl font-heading font-semibold text-text-primary mb-5">Announcements</h3>
		...
		<div>
			<h3 class="text-xl font-heading font-semibold text-text-primary mb-5">Need Help?</h3>
			<HelpSearchSection />
		</div>
	</div>
</template>
